...
if "output" in st.session_state:
    ...
    with col1:
        output_file_name = st.text_input(
            "Output file name",
            value="kegg_pathway_count_phenotype_output",
        )
...
if count_df is not None and not count_df.empty:
    st.markdown(
        '<div class="section-title">Save processed KEGG tables</div>',
        unsafe_allow_html=True,
    )

    st.markdown(
        '<div class="section-note">Optional: choose file names and save the full KEGG count table and top 10 table as TXT files.</div>',
        unsafe_allow_html=True,
    )
    ...
    with save_col1:
        full_count_file_name = st.text_input(
            "Full KEGG count table file name",
            value="kegg_pathway_count_table",
        )

    with save_col2:
        top10_file_name = st.text_input(
            "Top 10 KEGG table file name",
            value="kegg_top10_with_ties_table",
        )

    processed_output_directory = st.text_input(
        "Directory for processed KEGG tables",
        value=str(HERE),
    )

    save_tables_button = st.button("Save processed KEGG tables as TXT")
